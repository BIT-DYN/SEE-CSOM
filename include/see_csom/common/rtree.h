...
/// 实现了一种多维边界矩形树RTree。
///示例用法：对于三维树，请使用RTree<Object*，float，3>myTree；
///此修改后的模板化C++版本由Greg Douglas在Auran（http://www.auran.com)

///DATATYPE 引用数据应为int、void*、obj*等。不大于sizeof<void*>和简单类型
///ElemtType元素的类型，例如int或float
///NUMDIMS 维度
///ELEMTYPEREAL  允许在体积计算中使用分数和大值（如float或double）的元素类型
///
...
// 插入和删除数据需要知道其恒定的最小边界矩形。
// 这个版本对节点使用new/delete，为了提高效率，我建议使用固定大小的分配器。
// 我建议不要对返回的结果使用回调函数，而是使用类似于MFC CArray或STL Vector的高效的预大小、仅增长的内存数组来返回搜索结果。
...
protected: 
  ...
  struct Node;  // Fwd decl.  Used by other internal structs and iterator  由其他内部结构和迭代器使用

public:

  // These constant must be declared after Branch and before Node struct
  // Stuck up here for MSVC 6 compiler.  NSVC .NET 2003 is much happier.
  //这些常量必须在分支之后和节点结构之前声明
  enum
  {
    MAXNODES = TMAXNODES,                         ///< Max elements in node  节点中的最大元素数
    MINNODES = TMINNODES,                         ///< Min elements in node  节点中的最小元素数
  };
...
public:
  ...
  /// Insert entry 插入条目
  /// \param a_min Min of bounding rect  边界矩形的最小值
  /// \param a_max Max of bounding rect  边界矩形的最大值
  /// \param a_dataId Positive Id of data.  Maybe zero, but negative numbers not allowed.  数据的正Id。可能是零，但不允许负数。
  ...
  /// Find all within search rectangle  在搜索矩形中查找全部
  ...
  private:
    ...
    enum { MAX_STACK = 32 }; //  Max stack size. Allows almost n^32 where n is number of branches in node 最大堆栈大小。允许几乎n^32，其中n是节点中的分支数
  ...
  public:
    ...
    /// Is iterator invalid  迭代器无效
    bool IsNull()                                 { return (m_tos <= 0); }

    /// Is iterator pointing to valid data  迭代器是否指向有效数据
    bool IsNotNull()                              { return (m_tos > 0); }

    /// Access the current data element. Caller must be sure iterator is not NULL first. 访问当前数据元素。调用者首先必须确保迭代器不为NULL。
    DATATYPE& operator*()
    {
      ASSERT(IsNotNull());
    ...
    /// Find the next data element 查找下一个数据元素
    bool operator++()                             { return FindNextData(); }

    /// Get the bounds for this node 获取此节点的边界
  ...
  private:
    ...
    /// Find the next data element in the tree (For internal use only)  查找树中的下一个数据元素（仅供内部使用）
    bool FindNextData()
    {
      for(;;)
      {
        ...
        StackElement curTos = Pop(); // Copy stack top cause it may change as we use it 复制栈顶，因为它可能会改变我们使用它

        if(curTos.m_node->IsLeaf())
        { 
          // Keep walking through data while we can 尽可能地浏览数据
    ...
    StackElement m_stack[MAX_STACK];              ///< Stack as we are doing iteration instead of recursion  在执行迭代而不是递归时进行堆栈
    int m_tos;                                    ///< Top Of Stack index
  
    friend class RTree; // Allow hiding of non-public functions while allowing manipulation by logical owner
  };

  /// Get 'first' for iteration 获取迭代的“first”
...
protected:
  ...
  /// Minimal bounding rectangle (n-dimensional) 最小边界矩形（n维）
  struct Rect
  {
  ...
  struct Branch
  {
    Rect m_rect;                                  ///< Bounds  边界
    Node* m_child;                                ///< Child node  子节点
    DATATYPE m_data;                              ///< Data Id  数据id
  };

  /// Node for each branch level  每个分支级别的节点
  struct Node
  {
  ...
  /// A link list of nodes for reinsertion after a delete operation 删除操作后要重新插入的节点的链接列表
  struct ListNode
  {
  ...
  /// Variables for finding a split partition 用于查找分割分区的变量
...
RTREE_TEMPLATE
// 根据所给出的最大和最小，查看这里面是否有gp点，把点放在a_context中
...
// 在索引树或子树中搜索与参数矩形重叠的所有数据重定角。把数据放在a_context中
RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, t_resultCallback a_resultCallback, void* a_context)
{
  ...
  {
    // This is an internal node in the tree 这是树中的内部节点
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        ...
        {
          // The callback indicated to stop searching  指示停止搜索的回调
          return false;
        }
      }
    }
  ...
  return true; // Continue searching继续寻找
}
